...
`HELD_OUT_PROVENANCE = MIXED`: NUM-SYNTH and GEN-DESIGN remain process-recorded and not cryptographically proven; SYS-PROTOCOL includes a reproducible held-out hash manifest, but this public package still cannot prove historical sequestration or agent behavior retrospectively.
...
SYS-PROTOCOL public reproduction runs the three packaged candidates against the independent oracle, validates 25 held-out trace hashes, checks 12 mutation kills, and verifies the 36-row traceability matrix. This is a `PUBLIC_REPRODUCTION_RECORD`, distinct from the original execution record in `experiments/sys-protocol-001/gate1_report.md`.
